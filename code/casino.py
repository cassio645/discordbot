...
class Casino(commands.Cog):
    ...
    async def raspadinha(self, ctx, *, args='.'):
        if args.lower() == "comprar" or args.lower() == "compra":
            ...
            embed_raspadinha = discord.Embed(
				...
				description=f'É muito simples você deve usar o comando `{prefix}raspadinha comprar` para obter uma raspadinha. Lembrando que você pode ganhar nas linhas, colunas e diagonais.\n\n**Comando:** `{prefix}raspadinha comprar`ㅤㅤㅤㅤ**Preço:** 500 <:diamantt:973404655050719232>',
				colour=0xFFD301
			)
            embed_raspadinha.add_field(name="Prêmios", value=f"<:OWLgg:973214922655821864> 500<:diamantt:973404655050719232>\n<:dogecoin:973687932542148608> 1.000<:diamantt:973404655050719232> \n<:alienkiss:973238737867792435> 10.000<:diamantt:973404655050719232>\n<:bitcoin:973214832499236885> 30.000<:diamantt:973404655050719232>\n<:rubyheart:973214888539340810> 1.000.000<:diamantt:973404655050719232>")
    ...
    # slot-machine
    @commands.command(name="apostar", aliases=["aposta", "cassino"], cooldown_after_parsing=True)
    ...
    async def apostar(self, ctx, *, arg:int):
        ...
        if arg > 0:
            ...
            if resposta:
                ...
                if numero < 4:
                    premio = arg * 60
                    embed_jogo = discord.Embed(title="Jogando caça-niquel", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** -", colour=0xFFD301)
                    ...
                    novo_embed = discord.Embed(title="Parabéns você ganhou 60x", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** {premio} <:diamantt:973404655050719232>", colour=0x10c727)
                    ...
                    sleep(5)
                    add_money(ctx.author.id, premio)
                    novo_embed = discord.Embed(title="Parabéns você ganhou 25x", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** {premio} <:diamantt:973404655050719232>", colour=0x10c727)
                    ...
                    sleep(5)
                    add_money(ctx.author.id, premio)
                    novo_embed = discord.Embed(title="Parabéns você ganhou 10x", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** {premio} <:diamantt:973404655050719232>", colour=0x10c727)
                    ...
                    sleep(5)
                    add_money(ctx.author.id, premio)
                    novo_embed = discord.Embed(title="Parabéns você ganhou 5x", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** {premio} <:diamantt:973404655050719232>", colour=0x10c727)
                    ...
                    sleep(5)
                    add_money(ctx.author.id, premio)
                    novo_embed = discord.Embed(title="Parabéns você ganhou 2x", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** {premio} <:diamantt:973404655050719232>", colour=0x10c727)
                    ...
                    sleep(5)
                    novo_embed = discord.Embed(title="Que pena...", description=f"**Valor apostado:** {arg} <:diamantt:973404655050719232>\n**Prêmio:** 0", colour=0xc71010)
    ...
    async def apostar_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            if int(horario) > 99:
                await ctx.send(f'Você deve esperar mais {convert[3:5]}m para jogar novamemte.')
            else:
                await ctx.send(f'Você deve esperar mais {convert[-2:]}s para jogar novamemte.')
